...
from casestudies.dyn_sim.thesis_plot_style import (
    COLOR_COUPLED_ALT,
    COLOR_WIND,
    LS_REF,
    legend_compare,
...
def main(argv: list[str] | None = None):
    ...
    print(f"Using coupled CSV:  {fmu_csv}")
    ...
    if not os.path.exists(fmu_csv):
        raise FileNotFoundError(
            f"Missing coupled results CSV: {fmu_csv}\n"
    ...
    plots_dir = os.path.join(
        project_root, 'casestudies', 'dyn_sim', 'logs', 'compare', 'plots', 'thesis'
    )
    ...
    if 'P_e_sys_pu' in wt_a.columns and 'P_e_sys_pu' in fmu_a.columns:
        _save(
            'compare_P_e_sys_pu',
            'Electrical power at UIC',
            'Active power (p.u., system base)',
            baseline=(wt_a['P_e_sys_pu'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['P_e_sys_pu'].to_numpy(), legend_compare('coupled')),
        )
    if 'P_ref_sys_pu' in wt_a.columns and 'P_ref_sys_pu' in fmu_a.columns:
        _save(
            'compare_P_ref_sys_pu',
            'Active-power reference',
            'Power reference (p.u., system base)',
            baseline=(wt_a['P_ref_sys_pu'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['P_ref_sys_pu'].to_numpy(), legend_compare('coupled')),
            baseline_ls=LS_REF,
            coupled_ls=LS_REF,
        )

    q_act, q_ref = 'Q_uic_bus_actual_sys_pu', 'Q_uic_bus_ref_sys_pu'
    if q_act in wt_a.columns and q_act in fmu_a.columns:
        extra_q = []
        if q_ref in wt_a.columns:
            extra_q.append((
                wt_a[q_ref].to_numpy(),
                legend_compare('baseline', ref=True),
                COLOR_WIND,
                LS_REF,
            ))
        if q_ref in fmu_a.columns:
            extra_q.append((
                fmu_a[q_ref].to_numpy(),
                legend_compare('coupled', ref=True),
                COLOR_COUPLED_ALT,
                LS_REF,
            ))
        _save(
            'compare_Q_sys_pu',
            'Reactive power at UIC bus',
            'Reactive power (p.u., system base)',
            baseline=(wt_a[q_act].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a[q_act].to_numpy(), legend_compare('coupled')),
            extra=extra_q or None,
        )

    if 'v_bus_pu' in wt_a.columns and 'v_bus_pu' in fmu_a.columns:
        _save(
            'compare_Vt_pu',
            'UIC terminal voltage magnitude',
            'Voltage magnitude (p.u.)',
            baseline=(wt_a['v_bus_pu'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['v_bus_pu'].to_numpy(), legend_compare('coupled')),
        )
    ...
    if 'omega_e_pu_common' in wt_a.columns and 'omega_e_pu_common' in fmu_a.columns:
        _save(
            ...
            baseline=(wt_a['omega_e_pu_common'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['omega_e_pu_common'].to_numpy(), legend_compare('coupled')),
        )
    if 'omega_m_pu_common' in wt_a.columns and 'omega_m_pu_common' in fmu_a.columns:
        _save(
            ...
            baseline=(wt_a['omega_m_pu_common'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['omega_m_pu_common'].to_numpy(), legend_compare('coupled')),
        )

    if 'pitch_deg' in wt_a.columns and 'BldPitch1' in fmu_a.columns:
        _save(
            'compare_pitch_deg',
            'Blade pitch angle',
            'Pitch angle (deg)',
            baseline=(wt_a['pitch_deg'].to_numpy(), legend_compare('baseline')),
            coupled=(fmu_a['BldPitch1'].to_numpy(), legend_compare('coupled')),
        )

    wind_extra = []
    if 'RtVAvgxh' in fmu_a.columns:
        wind_extra.append((
            fmu_a['RtVAvgxh'].to_numpy(),
    ...
    if 'wind_speed_mps' in wt_a.columns:
        _save(
            ...
            'Wind speed',
            'Wind speed (m/s)',
            baseline=(wt_a['wind_speed_mps'].to_numpy(), legend_compare('baseline', detail='wind speed')),
            coupled=(fmu_a['Wind1VelX'].to_numpy(), legend_compare('coupled', detail='wind speed'))
